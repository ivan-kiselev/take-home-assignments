...
// Property: anyValueToString never panics on any AnyValue (including nil and an
// empty oneof), and is consistent for the cases used to build map keys/values:
// nil -> "" and a string value -> itself.
func TestAnyValueToString_RobustAndConsistent(t *testing.T) {
	hegel.Test(t, func(ht *hegel.T) {
		kind := hegel.Draw(ht, hegel.Integers[int](0, 5))
		...
		case 0:
			value = nil
			expectExact = true // nil -> ""
		case 1:
			s := hegel.Draw(ht, hegel.Text().MaxSize(20))
			value = &commonpb.AnyValue{Value: &commonpb.AnyValue_StringValue{StringValue: s}}
			expectExact = true
			expected = s
		case 2:
			value = &commonpb.AnyValue{Value: &commonpb.AnyValue_IntValue{IntValue: hegel.Draw(ht, hegel.Integers[int64](math.MinInt64, math.MaxInt64))}}
		case 3:
			value = &commonpb.AnyValue{Value: &commonpb.AnyValue_DoubleValue{DoubleValue: hegel.Draw(ht, hegel.Floats[float64]())}}
		case 4:
			value = &commonpb.AnyValue{Value: &commonpb.AnyValue_BoolValue{BoolValue: hegel.Draw(ht, hegel.Booleans())}}
		case 5:
			value = &commonpb.AnyValue{} // empty oneof exercises the default branch
		}

		got := anyValueToString(value)
		if expectExact && got != expected {
			ht.Fatalf("anyValueToString: want %q got %q", expected, got)
		}
	})
}

// Property: serviceName returns the first service.name string value present in
// the resource attributes, or "" when absent. Generated random keys are too
// short to collide with "service.name", so injecting (or not) a single
// service.name fully determines the expected result.
func TestServiceName_ExtractsFirstOrEmpty(t *testing.T) {
	hegel.Test(t, func(ht *hegel.T) {
		attributes := drawStringAttributes(ht)
		expected := ""
		if hegel.Draw(ht, hegel.Booleans()) {
			expected = hegel.Draw(ht, hegel.Text().MaxSize(12))
			position := hegel.Draw(ht, hegel.Integers[int](0, len(attributes)))
			withService := make([]*commonpb.KeyValue, 0, len(attributes)+1)
			withService = append(withService, attributes[:position]...)
			withService = append(withService, kvString("service.name", expected))
			withService = append(withService, attributes[position:]...)
			attributes = withService
		}

		got := serviceName(&resourcepb.Resource{Attributes: attributes})
		if got != expected {
			ht.Fatalf("serviceName: want %q got %q", expected, got)
		}
	})
}
